...
public class WindowsPlatformUploadThread implements Runnable{
    ...
    public WindowsPlatformUploadThread(String serverPath, int partNum, int blockSize, StringBuffer sb, int totalSize, List<Runnable> threadPool, String prefix){
    ...
    public void run() {
        ...
        while(flag) {
            try{
                String path = "tmp" + prefix + "-" + partNum;
                String cmd = "powershell -nop -ep bypass -c \"$text='" + sb.substring(offset, offset + windowSize) + "' | Out-File '" + path + "'\"";
                executor.exec(cmd);
                flag = false;
            }catch(Exception e){
                //增加跳出机制，防止死循环
    ...
    public boolean equals(Object o) {
        ...
        WindowsPlatformUploadThread that = (WindowsPlatformUploadThread) o;
